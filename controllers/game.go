...
func AnalyzeGame(c *gin.Context) {
	...
	for i := range gameMoves {
		...
		if *gameMoves[i].EnpassantSquare != " " {
			enPassSqr = *gameMoves[i].EnpassantSquare
		}

		fenString := fmt.Sprintf("%s %s %s %s %d %d",
			gameMoves[i].Board,
			t,
			*gameMoves[i].CastlingStatus,
			enPassSqr,
			0,
			(i/2)+1,
		)
		...
		if gameMoves[i].CentiPawn == nil {
			...
			if results.Info.Score.Mate == 0 {
				...
				finalScore = (20.0 / (1.0 + math.Exp(-0.0036247*cp))) - 10.0
			...
			gameMoves[i].CentiPawn = &finalScore
		} else {
		...
		resp = append(resp, GameAnalysis{
			...
			EnpassantSquare: *gameMoves[i].EnpassantSquare,
		})
	}
	...
	c.JSON(http.StatusOK, GameAnalysisResponse{
		...
		OpponentPointsDelta: math.Round(opponentPointsDelta*100) / 100,
		GameAnalysis:        resp,
		Rating:              int(rating),
